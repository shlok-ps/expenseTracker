import axios from "axios" import { IAppContext } from "src/context/AppContext"
...
export const analyzeSMS = async (appContext: IAppContext, sms: string) => {
  try {
    console.log("Using AI:", appContext.value, "with model:", appContext.model);
    if (appContext.value === 'OLLAMA') {
      const res = await axios.post(appContext.AI_BASE_URL, {
    ...
    } else if (appContext.value === 'CHATGPT') {
